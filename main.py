...
# Pandas base config
df = pd.read_csv('Estabelecimento.csv', encoding='ISO-8859-1', sep=',').fillna("")
...
def addValue():
    ...
    while True:
        ...
        if opUser == "Y":
            ...
            updateDf.to_csv('Estabelecimento.csv', sep=';', encoding='utf-8', index=False)
            break
...
def updateValue():
    ...
    #  CRIA UM NOVO DATAFRAME COM TODAS AS LINHAS DA COLUNA ESPECIFICADA PELO USUÁRIO
    ndf = pd.DataFrame(df[df.iloc[:, int(opUser) - 1].name][:opLimit])

    #  DA REPLACE EM TODOS OS VALORES QUE O USUÁRIO QUISER (VALORES IGUAIS)
    ndf.replace([inputValue], 'TESTE', inplace=True)

    #  PRINTA O DATAFRAME COM AS NOVAS ALTERAÇÕES
...
def deleteValue():
    ...
    while True:
        ...
        if hasContinue == "Y":
            newDf = newDf.drop(deleteChoice)
            print(tabulate(data(newDf[df.iloc[:, int(opUser) - 1].name][:opLimit]), headers='keys', tablefmt='pretty'))
            continue
        else:
            newDf = newDf.drop(deleteChoice)
            print('Those are like.')
            print(tabulate(data(newDf[df.iloc[:, int(opUser) - 1].name][:opLimit]), tablefmt='pretty'))
